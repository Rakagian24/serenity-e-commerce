...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    const res = await signIn("credentials", {
      ...
      password,
      redirect: false,
      callbackUrl: "/", // boleh diubah sesuai halaman tujuan
    ...
    if (res?.ok) {
      router.push(res.url); // ✅ Redirect manual
    } else {
            ...
            <div>
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full px-4 py-3 pr-12 rounded-xl border border-emerald-200 text-gray-700 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 focus:outline-none transition-all duration-300 bg-emerald-50/30"
                  required
                />
                <button
                  type="button"
                  onClick={togglePasswordVisibility}
                  className="absolute inset-y-0 right-0 pr-3 flex items-center text-emerald-400 hover:text-emerald-600 transition-colors duration-200"
                >
                  {showPassword ? (
